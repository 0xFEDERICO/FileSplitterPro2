...
/**
 * @author federico
 * Classe per comprimere e cifrare le singole parti splittate per una data dimensione di byte.
 */
public class ChunkSizeSplitterCompressCrypt extends ChunkSizeSplitterCompress {
	private String password;

	/**
	 * Costruttore della classe ChunkSizeSplitterCompressCrypt che inizializza gli
	 * attributi.
	 * 
	 ...
	 */
	public ChunkSizeSplitterCompressCrypt(String sf, String df, int parts, long bytes, String password,
			MainPanel main) {
	...
	/**
	 * Metodo esecutivo per comprimere e cifrare le parti divise per una data
	 * dimensione di bytes.
	 */
	@Override
	public void run() {
			...
			try {
				...
				if (i == 1) {
					fis_Source.skip(skip = ((long) HeaderService.getHeaderDim(fis_Source, main))); // skippo l'header, i
																									...
																									// da
																									// getHeaderDim()
					HeaderService.setHeader(new Header(totParts, i, true, true, this.fileName), fos_Dest, main);
